...
The results of the robustness analysis can be seen in figures 10 and 11. Initially, the number of nodes deleted was compared to the number of disconnected components that are created. This is obviously not a fair assessment. Otherwise it would be easy to get a large impact by cutting off the second station at the tail end of every route, thus creating a disconnected component in the first station. The analysis was therefore expanded to take into account the affected percentage of trips in each component that was disconnected from the network. 

![image](https://user-images.githubusercontent.com/101331875/170490024-06b76fda-d625-4833-a04a-1adbe635bc35.png)

Figures 10 & 11: Robustness based on disconnected components (top) and percentage of impacted trips (bottom)

Focussing on the percentage of impacted trips it becomes clear that targetting the most central stations has the most impact on the network. Below are the stations that get removed for both weighted and unweighted betweenness (note that we update betweenness after removing each node, so these nodes will be different compared to the overall highest centrality nodes). 

![image](https://user-images.githubusercontent.com/101331875/170496344-17ad0651-d414-4a8c-bd6f-26fc662352ee.png)

In bold are the breakpoints at which large jumps happen in the percentage of disconnected trains. Looking at weighted betweenness centrality, the first part to get disconnected from the network is upper Manhattan, Harlem and The Bronx. This is possible by removing only Lexington Avenue (4th) and 59 st (5d). The second large jump happens when Brooklyn, Queens and downtown Manhattan get disconnected. This requires more nodes to be deleted, mainly due to the fairly robust nature of the transfer stations in Brooklyn (see before). However, there are still some weak points in that part of the network. When we look at unweighted betweenness, the first part to get removed from the network in this case is a large part of Brooklyn and queens. This is made possible by deleting Court Sq and Broadway Junction from the network. It is also worth noting that this part of the network is responsible for the traffic from JFK airport to Manhattan, further increasing the importance of robustness in this part of the network. The second breakpoint using the unweighted betweenness is again cutting off The Bronx and Harlem from the rest of Manhattan. 

The results do stress the importance of the current ongoing Second Avenue project, putting more robustness in the network between The Bronx and Manhattan. A next important step would be to increase the robustness of the Queens and eastern part of Brooklyn. Broadway Junction and Court square are especially fragile parts in the system.

# Discussion

We investigated the New York subway network by creating a weighted graph using information on the station complexes, the routes and the weekly number of trips done throughout the network. We find that the New York subway network is fairly robust in some parts, but more fragile in others. The initial part of the analysis shows that lower Manhattan and Brooklyn (closer to Manhattan) have a lot of stations with high connectivity within and between their communities. The Bronx, upper Manhattan, Queens and the more distant parts of Brooklyn on the other hand are more fragile, having a couple of very central nodes with low degree. This clearly supports the need for the second avenue project that is currently happening to increase the accessibility of these neighbourhoods to downtown Manhattan, additionally, the network would be well of with more transfer stations in some of these locations to further increase the robustness of the network.
